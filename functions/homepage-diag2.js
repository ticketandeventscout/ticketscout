// functions/homepage-diag2.js — DIAGNOSTIC ONLY, maps to /homepage-diag2
// =============================================================================
// Follow-up to homepage-diag.js's result: env.ASSETS.fetch('/index.html')
// returned an unfollowed 308 (not an error, not a 200) — the same
// canonicalization confirmed live in-browser (/index.html -> /). This
// tests requesting the CANONICAL path "/" directly instead, which should
// bypass that redirect entirely and return real content with a 200 —
// verify before trusting, same as every step in this saga so far.
//
// Deploy at a THIRD disposable path, alongside (not instead of)
// homepage-diag.js and homepage-test.js. Still not a homepage candidate.
// =============================================================================
...
export async function onRequestGet({ request, env }) {
  // DIAGNOSTIC MODE — this variant exists purely to find out what actually
  // happened, since env.ASSETS.fetch() also fell through to the fallback
  // and there is no way to tell WHY from the outside. Never intended to
  // become the real homepage — see homepage-test.js for that candidate,
  // paused until this tells us what to fix.
  const diag = { envAssetsExists: false, envAssetsType: typeof env.ASSETS, fetchAttempted: false, fetchError: null, responseStatus: null, responseOk: null, theory: 'requesting / (canonical) via env.ASSETS instead of /index.html (non-canonical, which returned an unfollowed 308 last time)' };
  try {
    diag.envAssetsExists = !!(env.ASSETS && typeof env.ASSETS.fetch === 'function');
    if (diag.envAssetsExists) {
      diag.fetchAttempted = true;
      // CHANGED from /index.html to / — the diagnostic run just proved
      // /index.html returns an unfollowed 308 (canonicalizes to /), so
      // request the canonical path directly instead of the one already
      // confirmed to redirect away from itself.
      const assetUrl = new URL('/', request.url);
      const resp = await env.ASSETS.fetch(new Request(assetUrl.toString(), request));
      diag.responseStatus = resp.status;
      diag.responseOk = resp.ok;
      if (resp.ok) {
...
async function _unused_onRequestGet({ request, env }) {
  try {
