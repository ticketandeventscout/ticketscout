// functions/homepage-diag.js — DIAGNOSTIC ONLY, maps to /homepage-diag
// =============================================================================
// Not a homepage candidate. env.ASSETS.fetch() (the Round 3 fix) also fell
// through to the safe fallback, and there's no way to tell from the
// outside WHY — could be env.ASSETS not existing on this Pages project at
// all, or existing but throwing, or existing and returning a non-ok
// status. This returns exactly which of those happened as visible JSON,
// so the next fix is built on a real answer instead of a third guess.
//
// Deploy this ALONGSIDE homepage-test.js (round 3), not instead of it —
// different path (/homepage-diag vs /homepage-test), both disposable,
// neither touches the real homepage.
// =============================================================================
...
export async function onRequestGet({ request, env }) {
  ...
  const diag = { envAssetsExists: false, envAssetsType: typeof env.ASSETS, fetchAttempted: false, fetchError: null, responseStatus: null, responseOk: null };
  try {
