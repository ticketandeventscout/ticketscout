...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    if (ekv) {
      const m = await ekv.get(`entity:meta:football:${team.slug}`);
      if (m) {
        const meta = JSON.parse(m);
        enrichFacts = meta.facts || null;
        const generic = !team.description || /^Compare .{0,80} ticket prices across verified sellers/.test(team.description);
        if (meta.aboutText && generic) team.description = meta.aboutText;
      }
    }
  } catch {}
